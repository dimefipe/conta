/* =========================================================
   ...
   - ✅ Adds journal entry templates module:
       - entry_templates (header)
       - entry_template_lines (lines)
       - Foreign keys + indexes (safe)
...
BEGIN
  ...
  FROM information_schema.REFERENTIAL_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'entry_templates'
    AND CONSTRAINT_NAME = 'fk_et_company';

  IF fk_exists = 0 THEN
    ALTER TABLE entry_templates
      ADD CONSTRAINT fk_et_company FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE CASCADE;
  END IF;
  ...
  WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'entry_templates'
    AND CONSTRAINT_NAME = 'fk_et_created_by';

  IF fk_exists = 0 THEN
    ALTER TABLE entry_templates
      ADD CONSTRAINT fk_et_created_by FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL;
  END IF;
  ...
  FROM information_schema.REFERENTIAL_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'entry_template_lines'
    AND CONSTRAINT_NAME = 'fk_etl_template';

  IF fk_exists = 0 THEN
    ALTER TABLE entry_template_lines
      ADD CONSTRAINT fk_etl_template FOREIGN KEY (template_id) REFERENCES entry_templates(id) ON DELETE CASCADE;
  END IF;
  ...
  WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'entry_template_lines'
    AND CONSTRAINT_NAME = 'fk_etl_account';

  IF fk_exists = 0 THEN
    ALTER TABLE entry_template_lines
      ADD CONSTRAINT fk_etl_account FOREIGN KEY (account_id) REFERENCES accounts(id);
  END IF;
