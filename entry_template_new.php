...
function validate_lines(array $post): array {
  ...
  for ($i=0; $i<count($accs); $i++) {
    ...
    if ($account_id <= 0) continue; // ignora filas vacías

    // regla: solo uno de los dos
    if ($debit > 0 && $credit > 0) {
      return [[], 'Una línea no puede tener Debe y Haber al mismo tiempo.'];
    }
    if ($debit <= 0 && $credit <= 0) {
      return [[], 'Cada línea debe tener Debe o Haber.'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($name === '') {
    ...
    redirect('entry_template_new.php');
  }

  [$lines, $err] = validate_lines($_POST);
  if ($err) {
    flash_set('err', $err);
    redirect('entry_template_new.php');
  }

  // Validar que las cuentas pertenezcan a la empresa actual
  ...
  $st = $pdo->prepare("
    INSERT INTO entry_templates (company_id, name, description, created_by, created_at)
    VALUES (?, ?, ?, ?, NOW())
  ");
  $st->execute([$cid, $name, ($description !== '' ? $description : null), (int)($user['id'] ?? 0)]);
  ...
  flash_set('ok', 'Plantilla creada correctamente.');
...
<div class="card">
  <h2 style="margin:0 0 10px;">Nueva plantilla</h2>
  <div class="small">Crea una plantilla cuadrada (Debe = Haber) para reutilizarla en nuevos asientos.</div>
</div>
...
<form method="post" class="card">
  ...
  <div class="row">
    <div class="field" style="flex:1;min-width:260px;">
      <label>Nombre</label>
      <input name="name" placeholder="Ej: Pago de proveedores" required>
    </div>

    <div class="field" style="flex:2;min-width:260px;">
      <label>Descripción (opcional)</label>
      <input name="description" placeholder="Ej: Caja/Banco vs Proveedores + IVA…">
    </div>
  </div>
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-top:12px;">
    ...
    <div style="display:flex;gap:10px;">
      ...
      <button class="btn" id="saveEntry" type="submit">Guardar plantilla</button>
    </div>
  </div>
</form>
